...
export default function UsersPage() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      // FIX: only show customers (role === 'user'), not restaurant owners or admins
  ...
  if (loading) return (
    <>
      <style>{styles}</style>
      <div className="p-loading"><div className="p-spinner" /><span>Loading users...</span></div>
    </>
  )
    ...
    <>
      ...
      <div className="p-root">
        <header className="p-header">
          ...
          <div className="p-chip">{users.length} customers</div>
        </header>
                  ...
                  <tr key={u.id}>
                    ...
                    <td>
                      <div className="p-user-cell">
                        <div className="p-avatar">{u.name?.[0]?.toUpperCase() || '?'}</div>
                        <span className="td-bold">{u.name}</span>
                      </div>
                    </td>
                    <td className="td-muted">{u.email}</td>
                    <td className="td-muted">{u.phone || <span className="td-nil">—</span>}</td>
                    <td>
                      {u.isVerified
